...
int check_line(int(*sudoku)[9], int sdk_index);
...
void P(int** p_sdk_part, int** p_nums, int nums_index, int(*sudoku)[9], int sdk_index)
{
	...
	if (nums_index == 9)
	{
		...
		if (check_line(sudoku, sdk_index) == 1) P(p_sdk_part, p_nums, 0, sudoku, sdk_index + 1);
...
int check_line(int(*sudoku)[9], int sdk_index)
{
	...
	case 2:
		for (i = 0; i < 3; i++)
		{
			...
			if (mul != 362880) return 0; // 362880 = 1 * 2 * 3 * 4 * 5 * 6 * 7 * 8 * 9
		}
		break;
	case 5:
		for (i = 0; i < 3; i++)
		{
			...
			for (j = 0; j < 9; j++)
			{
				mul = mul * sudoku[i + 3][j];
			}
			if (mul != 362880) return 0;
		}
		break;
	case 6:
		for (i = 0; i < 3; i++)
